...
mod tests {
    ...
    async fn test_protocols() -> anyhow::Result<()> {
        let (machine, manager) = get_paired_machine_and_manager().await?;

        // Wait for claim to finish.
        for _ in 0..10 {
            if manager.list_machines().unwrap().len() == 1 {
                break;
            }
            tokio::time::sleep(Duration::from_millis(100)).await;
        }

        assert_eq!(manager.list_machines().unwrap().len(), 1);

        assert_eq!(machine.get_machine_config().await?, None);
        ...
        let machine_id = manager.list_machines().unwrap()[0].0;
        manager
        ...
        for i in 0..10 {
            if let Ok(Some(_)) = machine.get_machine_config().await {
                break;
            }
        ...
        // Wait for the machine protocol to receive the invite code.
        for i in 0..10 {
            if manager.get_claimable_contracts().await.is_ok() {
                break;
            }
        ...
        let claimable_contracts = manager.get_claimable_contracts().await?;
        assert_eq!(manager.get_claimable_contracts().await?.len(), 1);
    ...
    async fn test_claim_rejected_by_machine() -> anyhow::Result<()> {
        let (machine, manager) = get_machine_and_manager(false, true).await?;

        tokio::time::sleep(Duration::from_millis(100)).await;

        assert_eq!(manager.list_machines()?.len(), 0);

        // After rejection, another claim should be possible
        let machine_addr = machine.node_addr().await?;
        let manager_task = tokio::spawn(async move {
            let (pin_mgr, tx_mgr) = manager.claim_machine(machine_addr).await.unwrap();
            tx_mgr.send(true).unwrap();
            (pin_mgr, manager)
        });
        let machine_task = tokio::spawn(async move {
            let (pin_machine, tx_machine) =
    ...
    #[tokio::test]
    async fn test_list_machines_on_new_manager() -> anyhow::Result<()> {
        let manager_storage_path = tempfile::tempdir()?;
        let manager = ManagerProtocol::new(manager_storage_path.path()).await?;

        assert!(manager.list_machines()?.is_empty());
    ...
    async fn test_shutdown_idempotency() -> anyhow::Result<()> {
        // Shutdown machine first.
        let (machine_protocol, manager_protocol) = get_paired_machine_and_manager().await?;
        for _ in 0..10 {
            machine_protocol.shutdown().await;
        ...
        // Shutdown both back and forth, starting with machine.
        let (machine_protocol, manager_protocol) = get_paired_machine_and_manager().await?;
        for _ in 0..10 {
            machine_protocol.shutdown().await;
            manager_protocol.shutdown().await;
        ...
        // Shutdown both back and forth, starting with manager.
        let (machine_protocol, manager_protocol) = get_paired_machine_and_manager().await?;
        for _ in 0..10 {
            manager_protocol.shutdown().await;
            machine_protocol.shutdown().await;
    ...
    async fn get_paired_machine_and_manager() -> anyhow::Result<(MachineProtocol, ManagerProtocol)>
    {
        let (machine, manager) = get_machine_and_manager(true, true).await?;
    ...
    async fn get_machine_and_manager(
        claim_response_machine: bool,
        claim_response_manager: bool,
    ) -> anyhow::Result<(MachineProtocol, ManagerProtocol)> {
        let machine_storage_path = tempfile::tempdir()?;
        let machine_protocol = MachineProtocol::new(machine_storage_path.path()).await?;
        ...
        let machine_task = tokio::spawn(async move {
            let (pin, tx) = machine_protocol
                ...
                .await
                .unwrap();
            tx.send(claim_response_machine).unwrap();
            (pin, machine_protocol)
        });

        let manager_task = tokio::spawn(async move {
            let (pin, tx) = manager_protocol.claim_machine(machine_addr).await.unwrap();
            tx.send(claim_response_manager).unwrap();
            (pin, manager_protocol)
        });
        ...
        Ok((machine_protocol, manager_protocol))
    }
}
